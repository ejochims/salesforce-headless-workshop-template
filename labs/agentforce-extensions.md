...
Use these modules after the live hour from the participant-owned local Salesforce project. Agentforce work should continue from the approved Operations Support Assistant Agent Spec.
...
- `sf agent validate authoring-bundle --json --api-name Operations_Support_Assistant` succeeds.
...
```text
Create an Agentforce test suite for Operations Support Assistant. Include happy path partner lookup, delivery shortage, missing data, no matching partner, off-topic handling, and Case creation recommendation examples.
```
...
```text
Create or update a permission set for workshop users with access to Partner__c, Delivery__c, standard Case operations fields, deliveryTracker, and any Agentforce Employee Agent access needed for preview.
```
